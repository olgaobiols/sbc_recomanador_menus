...
(deftemplate beguda-valida-alcohol-no (slot nom))
(deftemplate beguda-valida-formal (slot nom))
(deftemplate beguda-valida-maridatge (slot nom))
(deftemplate beguda-general (slot nom))
(deftemplate beguda-primer (slot nom))
(deftemplate beguda-segon (slot nom))
(deftemplate beguda-postres (slot nom))
...
(defrule AbstraccioHeuristica::filtrar-begudes-alcohol-flexible
...
=>
   (if (eq ?alc no) then
       ;; Si NO vol alcohol, només accepta begudes sense alcohol
       (if (eq ?alcohol no) then
           (assert (beguda-valida-alcohol (nom ?nom)))
       )
   else
       (if (eq ?alcohol no) then
        (assert (beguda-valida-alcohol-no (nom ?nom)))
       else
        (assert (beguda-valida-alcohol (nom ?nom)))
       )
   )
...
(defrule AssociacioHeuristica::filtrar-begudes-per-mode
   (declare (auto-focus TRUE))
   ?p <- (peticio (beguda-mode ?bm))
   ?beguda <- (object (is-a Beguda)
            (nom ?nom)
            (maridatge $?m))
   ...
   (if (eq ?bm general) then
       (if (>= (length$ $?m) 2) then
           (assert (beguda-general (nom ?nom)))
       )
    else
       (progn
         (if (member$ ordre-primer $?m) then
             (assert (beguda-primer (nom ?nom)))
         )
         (if (member$ ordre-segon $?m) then
             (assert (beguda-segon (nom ?nom)))
         )
         (if (member$ ordre-postres $?m) then
             (assert (beguda-postres (nom ?nom)))
         )
       )
   )
)
   ...
   (if (<= ?limit 0)
      then
         ...
         (loop-for-count (?i 1 ?limit)
            ;; 🟦 Reiniciem control de begudes per menú
            (bind ?usades (create$))
            ...
            ;; -----------------------
            ;; BEGUDES SEGONS EL MODE
            ;; -----------------------
            (if (eq ?bm general)
               then
                  ;; MODE GENERAL
                  (bind ?begudes (find-all-facts ((?b beguda-general)) TRUE))
                  (bind ?numB (length$ ?begudes))
                  (if (> ?numB 0)
                     then
                        (bind ?index (mod (- ?i 1) ?numB))
                        (bind ?beguda (nth$ (+ ?index 1) ?begudes))
                        (bind ?nomB (fact-slot-value ?beguda nom))
                        (bind ?objB (nth$ 1 (find-instance ((?x Beguda))
                                             (eq (send ?x get-nom) ?nomB))))
                        (if (neq ?objB FALSE)
                           then (bind ?preuB (send ?objB get-preu_cost))
                           else (bind ?preuB 0))
                     else
                        (bind ?nomB "Cap beguda recomanada")
                        (bind ?preuB 0)
                  )
                  ...
                  ;; MODE PER-PLAT
                  ;; PRIMER PLAT
                  (bind ?begudes-primer (find-all-facts ((?b beguda-primer)) TRUE))
                  (bind ?candidates (create$))
                  (foreach ?b ?begudes-primer
                    (if (not (member$ (fact-slot-value ?b nom) ?usades))
                        then (bind ?candidates (create$ $?candidates ?b))))
                  (if (= (length$ ?candidates) 0) then (bind ?candidates ?begudes-primer))
                  (bind ?numBpr (length$ ?candidates))
                  (if (> ?numBpr 0)
                    then
                        (bind ?index-pr (mod (- ?i 1) ?numBpr))
                        (bind ?bpr (nth$ (+ ?index-pr 1) ?candidates))
                        (bind ?nomBpr (fact-slot-value ?bpr nom))
                        (bind ?objBpr (nth$ 1 (find-instance ((?x Beguda))
                                            (eq (send ?x get-nom) ?nomBpr))))
                        (if (neq ?objBpr FALSE)
                          then (bind ?preuBpr (send ?objBpr get-preu_cost))
                          else (bind ?preuBpr 0))
                        (bind ?usades (create$ $?usades ?nomBpr))
                    else
                        (bind ?nomBpr "Cap beguda recomanada")
                        (bind ?preuBpr 0)
                  )

                  ;; SEGON PLAT
                  (bind ?begudes-segon (find-all-facts ((?b beguda-segon)) TRUE))
                  (bind ?candidates (create$))
                  (foreach ?b ?begudes-segon
                    (if (not (member$ (fact-slot-value ?b nom) ?usades))
                        then (bind ?candidates (create$ $?candidates ?b))))
                  (if (= (length$ ?candidates) 0) then (bind ?candidates ?begudes-segon))
                  (bind ?numBsg (length$ ?candidates))
                  (if (> ?numBsg 0)
                    then
                        (bind ?index-sg (mod (- ?i 1) ?numBsg))
                        (bind ?bsg (nth$ (+ ?index-sg 1) ?candidates))
                        (bind ?nomBsg (fact-slot-value ?bsg nom))
                        (bind ?objBsg (nth$ 1 (find-instance ((?x Beguda))
                                            (eq (send ?x get-nom) ?nomBsg))))
                        (if (neq ?objBsg FALSE)
                          then (bind ?preuBsg (send ?objBsg get-preu_cost))
                          else (bind ?preuBsg 0))
                        (bind ?usades (create$ $?usades ?nomBsg))
                    else
                        (bind ?nomBsg "Cap beguda recomanada")
                        (bind ?preuBsg 0)
                  )

                  ;; POSTRES
                  (bind ?begudes-postres (find-all-facts ((?b beguda-postres)) TRUE))
                  (bind ?candidates (create$))
                  (foreach ?b ?begudes-postres
                    (if (not (member$ (fact-slot-value ?b nom) ?usades))
                        then (bind ?candidates (create$ $?candidates ?b))))
                  (if (= (length$ ?candidates) 0) then (bind ?candidates ?begudes-postres))
                  (bind ?numBpo (length$ ?candidates))
                  (if (> ?numBpo 0)
                    then
                        (bind ?index-po (mod (- ?i 1) ?numBpo))
                        (bind ?bpo (nth$ (+ ?index-po 1) ?candidates))
                        (bind ?nomBpo (fact-slot-value ?bpo nom))
                        (bind ?objBpo (nth$ 1 (find-instance ((?x Beguda))
                                            (eq (send ?x get-nom) ?nomBpo))))
                        (if (neq ?objBpo FALSE)
                          then (bind ?preuBpo (send ?objBpo get-preu_cost))
                          else (bind ?preuBpo 0))
                        (bind ?usades (create$ $?usades ?nomBpo))
                    else
                        (bind ?nomBpo "Cap beguda recomanada")
                        (bind ?preuBpo 0)
                  )
            )
            ...
            (bind ?total
               (if (eq ?bm general)
                  then (+ ?ppr ?psg ?ppo ?preuB)
                  else (+ ?ppr ?psg ?ppo ?preuBpr ?preuBsg ?preuBpo))
            )
            ...
            (if (eq ?bm general)
               then
                  (printout t "  Beguda Recomanada: " ?nomB "  [" ?preuB " €]" crlf)
               else
                  (printout t "  Beguda Entrant:   " ?nomBpr "  [" ?preuBpr " €]" crlf)
                  (printout t "  Beguda Principal: " ?nomBsg "  [" ?preuBsg " €]" crlf)
                  (printout t "  Beguda Postres:   " ?nomBpo "  [" ?preuBpo " €]" crlf)
            )
