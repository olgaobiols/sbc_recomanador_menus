...
; (deftemplate abstraccio-done
;   (slot ok (default TRUE)))

; (deftemplate refinament-done
;   (slot ok (default TRUE)))
...
;; -------------------- MAPES AL·LÈRGENS / DIETES --------------------
(deffunction mapa-alergens (?n)
  (bind ?i (integer ?n))
  (if (eq ?i 1)  then (return (create$ "Gluten")))
  (if (eq ?i 2)  then (return (create$ "Llet")))
  (if (eq ?i 3)  then (return (create$ "Ous")))
  (if (eq ?i 4)  then (return (create$ "Peix")))
  (if (eq ?i 5)  then (return (create$ "Crustacis")))
  (if (eq ?i 6)  then (return (create$ "Mol·luscs")))
  (if (eq ?i 7)  then (return (create$ "Fruits secs")))
  (if (eq ?i 8)  then (return (create$ "Cacauet")))
  (if (eq ?i 9)  then (return (create$ "Soja")))
  (if (eq ?i 10) then (return (create$ "Api")))
  (if (eq ?i 11) then (return (create$ "Mostassa")))
  (if (eq ?i 12) then (return (create$ "Sèsam")))
  (if (eq ?i 13) then (return (create$ "Sulfits")))
  (if (eq ?i 14) then (return (create$ "Tramussos")))
  (create$)
)

;; 1 Vegà, 2 Vegetarià, 3 Halal, 4 Kosher, 5 Sense gluten, 6 Sense llet, 7 Sense ous, 8 Sense soja
(deffunction mapa-dieta (?n)
  (bind ?i (integer ?n))
  (if (eq ?i 1) then (return (create$ VG)))
  (if (eq ?i 2) then (return (create$ V)))
  (if (eq ?i 3) then (return (create$ HALAL_POT)))
  (if (eq ?i 4) then (return (create$ KOSHER_POT)))
  (if (eq ?i 5) then (return (create$ GF)))
  (if (eq ?i 6) then (return (create$ DF)))
  (if (eq ?i 7) then (return (create$ EF)))
  (if (eq ?i 8) then (return (create$ SF)))
  (create$)
)
...
=>
  (printout t crlf "Llista d'al·lèrgens (EU-14). Escriu els números separats per espais:" crlf)
  (printout t " 1 Gluten | 2 Llet | 3 Ous | 4 Peix | 5 Crustacis | 6 Mol·luscs | 7 Fruits secs" crlf)
  (printout t " 8 Cacauet | 9 Soja | 10 Api | 11 Mostassa | 12 Sèsam | 13 Sulfits | 14 Tramussos" crlf crlf)
  (printout t "Exemple: 1 2 7" crlf)
  (bind ?raw (readline))
  (bind ?labels (map-multi-idx ?raw map-alergen-idx->labels))
  (modify ?p (alergens-prohibits ?labels))
  (assert (alergens-detallats))
  (printout t "Has indicat com a PROHIBITS: " ?labels crlf))

(defrule PreferenciesMenu::detallar-dietes
  ?p <- (peticio (alergies-si si))
  (alergens-detallats)
  (not (dietes-detallades))
=>
  (printout t crlf "Dietes/condicions especials. Escriu els números separats per espais (o buit si cap):" crlf)
  (printout t " 1 Vegà | 2 Vegetarià | 3 Halal | 4 Kosher | 5 Sense gluten | 6 Sense llet | 7 Sense ous | 8 Sense soja" crlf)
  (bind ?raw (readline))
  (bind ?codes (map-multi-idx ?raw mapa-dieta-idx->codes))
  (modify ?p (dietes-requerides ?codes))
  (assert (dietes-detallades))
  (printout t "Has indicat com a REQUERIDES: " ?codes crlf))
...
(deftemplate plat-valid-temp  (slot nom))
(deftemplate plat-valid-complexitat (slot nom))
(deftemplate plat-valid-formal (slot nom))
(deftemplate plat-valid-alergens (slot nom))
(deftemplate plat-valid-dietes   (slot nom))
...
(defrule AbstraccioHeuristica::enllacar-plats-ingredients
  (declare (salience 100))
  ?P <- (object (is-a Plat) (te_ingredients_noms $?L))
  ?I <- (object (is-a Ingredient) (nom ?ni))
  (test (member-ci$ ?ni $?L))  ;; si no fas servir els helpers, substitueix per: (test (member$ ?ni ?L))
  (test (not (member$ ?P (send ?I get-part_de)))) ;; evita duplicats si ja està enllaçat
=>
  (send ?I put-part_de (create$ (send ?I get-part_de) ?P))
  (send ?P put-te_ingredients (create$ (send ?P get-te_ingredients) ?I))
)

;; Al·lèrgens
(defrule AbstraccioHeuristica::alergens-sense-restriccio
   (plat-valid-temp (nom ?nom))
   (peticio (alergens-prohibits $?AP))
   (test (eq (length$ ?AP) 0))
 =>
   (assert (plat-valid-alergens (nom ?nom)))
)

(defrule AbstraccioHeuristica::filtrar-plats-per-alergens
   (plat-valid-temp (nom ?nom))
   (peticio (alergens-prohibits $?AP&:(> (length$ ?AP) 0)))
 =>
   (bind ?plats (find-all-instances ((?p Plat)) (eq (send ?p get-nom) ?nom)))
   (if (> (length$ ?plats) 0) then
     (bind ?P (nth$ 1 ?plats))
     (bind ?ings (find-all-instances ((?i Ingredient)) (member$ ?P (send ?i get-part_de))))
     (bind ?apte TRUE)
     (foreach ?i ?ings
       (bind ?a (send ?i get-alergen))
       (if (and (neq ?a "-") (member$ ?a ?AP)) then (bind ?apte FALSE)))
     (if ?apte then (assert (plat-valid-alergens (nom ?nom))))))

;; Dietes
(defrule AbstraccioHeuristica::dietes-sense-restriccio
   (plat-valid-temp (nom ?nom))
   (peticio (dietes-requerides $?DR))
   (test (eq (length$ ?DR) 0))
 =>
   (assert (plat-valid-dietes (nom ?nom)))
)

(defrule AbstraccioHeuristica::filtrar-plats-per-dietes
   (plat-valid-temp (nom ?nom))
   (peticio (dietes-requerides $?DR&:(> (length$ ?DR) 0)))
 =>
   (bind ?plats (find-all-instances ((?p Plat)) (eq (send ?p get-nom) ?nom)))
   (if (> (length$ ?plats) 0) then
     (bind ?P (nth$ 1 ?plats))
     ...
     (foreach ?req ?DR
       (foreach ?i ?ings
         (bind ?d (send ?i get-dietes))
         (if (not (member$ ?req ?d)) then (bind ?apte FALSE))))
     (if ?apte then (assert (plat-valid-dietes (nom ?nom))))))


(defrule AbstraccioHeuristica::final-abstraccio
   (declare (auto-focus TRUE))
   (not (plat-pendent-temp))
   (not (abstraccio-done))   ;; evita duplicats
 =>
   (printout t ">> Final Abstracció: canviant focus a Associació" crlf)
   (assert (abstraccio-done))   ;; SENSE MAIN::
   (focus AssociacioHeuristica))
...
(defrule RefinamentHeuristica::combinar-validacions
   ...
   (plat-valid-alergens (nom ?nom))
   (plat-valid-dietes   (nom ?nom))
 =>
...
(defrule RefinamentHeuristica::final-refinament
   ...
   (not (refinament-done))   ;; evita duplicats
 =>
   (printout t ">> Final Refinament: canviant focus a Composició" crlf)
   (assert (refinament-done))   ;; SENSE MAIN::
   (focus ComposicioMenus))

;; -------------------- PAS 5: COMPOSICIÓ I SORTIDA --------------------
(defmodule ComposicioMenus
  (import MAIN ?ALL)
  (import PreferenciesMenu ?ALL)
  (export ?ALL))

;; Només imprimir MENÚS APTES (plats que han passat tots els filtres)
(defrule ComposicioMenus::mostrar-menus-aptes
   (declare (auto-focus TRUE))
   (respostes-completes)
   (refinament-done)            ;; ja s'han creat els plat-valid-final
   (not (menus-aptes-presentats))
 =>
   ;; Recollim noms de plats finals vàlids (ja filtrats per temperatura, al·lèrgens i dietes)
   ...
   ;; Busquem instàncies per ordre només dels que són vàlids
   ...
   (printout t crlf "===== MENUS APTES (segons les restriccions) =====" crlf)
   (if (<= ?limit 0) then
       (printout t "*** No hi ha cap combinació 100% apta amb les restriccions indicades." crlf)
       (printout t "    (Pots relaxar restriccions o permetre substitucions d'ingredients.)" crlf)
    else
       (printout t "Et proposem " ?limit " menus aptes:" crlf)
       (loop-for-count (?i 1 ?limit)
         (bind ?primer (nth$ ?i ?primers))
         (bind ?segon  (nth$ ?i ?segons))
         (bind ?postre (nth$ ?i ?postres))
         (printout t crlf "*** Menu apte " ?i " ***" crlf)
